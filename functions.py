...
def most_connected_nodes(g):
    ...
    #for i in range(len(g[0])):
        #deg.append(list(g[0].degree())[i][1])
    
    #meandeg = sum(deg)/float(len(g[0]))
    
    #connected_nodes = []
    #for node in g[0].nodes:
        #count = 0
        #for i in range(len(g)):
            #if g[i].degree(node)>1+meandeg:
                #count += 1
        #if count >= len(g)/float(2):
    ...
    #for node in g[0].nodes:
        #deg_tot[node] = 0
        #for i in range(50):
            #deg_tot[node] += g[i].degree(node)
    
    #sorted_x = sorted(deg_tot.items(), key=operator.itemgetter(1))
    #sorted_dict = collections.OrderedDict(sorted_x)
    
    #connected_nodes = sorted_dict.keys()[-int((len(g[0])*0.1)+0.5):]
    
    connected_nodes = list(numpy.random.choice(g[0].nodes,int((len(g[0])*0.1)+0.5)))
    
    return(connected_nodes)

def set_nodes_as_agents(gt,beta,mu,ex,mu_r,mu_d,d,fimm,frec):
    """Assigns to each agent(node of the network) an instance of the class "People" """
    from agents import people
    for j in range(len(gt)):
        gt[j].agent={}    
        for i in gt[j].nodes():
            gt[j].agent[i]=people(beta,mu,ex,mu_r,mu_d,d,fimm,frec)
...
def set_graph(gt,initial_infected,network_size):
    ...
    for i in range(network_size):
        pos.append((random.randrange(0, network_size), random.randrange(0, network_size)))
    return(seed, susceptible_nodes, infected_nodes, exposed_nodes, hr_nodes, hd_nodes, recovered_nodes, d_nodes, imm_nodes, rec_nodes, pos, color_map)


def dynamic_graph(edgelist,gt,beta,mu,ex,mu_r,mu_d,d,fimm,frec,infected_nodes):
    ...
    for i in G_dyn.nodes():
        G_dyn.agent[i]=people(beta,mu,ex,mu_r,mu_d,d,fimm,frec)
...
def update_compartments(network,imm_nodes,d_nodes):
    ...
    return(infected_nodes, susceptible_nodes, hr_nodes, hd_nodes,recovered_nodes,
           exposed_nodes,d_nodes,imm_nodes,rec_nodes)
...
def run_simulation(G_dyn, t, edgelist,infected_nodes, susceptible_nodes, hr_nodes, hd_nodes,recovered_nodes,
    ...
    for time in range(t):
        ...
        #most connected nodes immunization process:
        for i in connected_nodes:
            if G_dyn.agent[i].state=='s':
                G_dyn.agent[i].state='imm'
        
        #infection propagation by infected 
        for i in infected_nodes:
            for j in G_dyn.neighbors(i):
                if G_dyn.agent[j].state=='s':
                    G_dyn.agent[j].susceptible_exposed_byinfected()
            #infected->hospitalized_r
            G_dyn.agent[i].infected_hospetalized_r()
    
        #infected->hospitalized_d
        for i in infected_nodes:
            G_dyn.agent[i].infected_hospetalized_d()
        
        #infection propagation by hospitalized            
        for i in (hr_nodes+hd_nodes):
            for j in G_dyn.neighbors(i):
                if G_dyn.agent[j].state=='s':
                    G_dyn.agent[j].susceptible_exposed_byhospitalized()
        #exposed->infected
        ...
        #infection propagation by recovered
        if time < 270:
            for i in (recovered_nodes):
                for j in G_dyn.neighbors(i):
                    if G_dyn.agent[j].state=='s':
                        if time < 60:
                            G_dyn.agent[j].susceptible_exposed_byrecovered()
                        if (time >= 60 & time < 180):
        ...
        #recovered->recidive
        for x in (recovered_nodes):
            G_dyn.agent[x].recovered_recidive()
        
        #recidive->infected
        for x in (rec_nodes):
            G_dyn.agent[x].recidive_infected()
        ...
        infected_nodes, susceptible_nodes, hr_nodes, hd_nodes,recovered_nodes,exposed_nodes,d_nodes,imm_nodes,rec_nodes = update_compartments(G_dyn,imm_nodes,d_nodes)
        
        #for i in (recovered_nodes+rec_nodes):
            #color_map[i] = 'yellow'
        ...
        r_size.append(float(len(recovered_nodes+rec_nodes))/float(len(gt[0])))
